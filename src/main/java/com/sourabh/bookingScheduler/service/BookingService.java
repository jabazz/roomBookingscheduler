...
public class BookingService {
	...
	public static void confirmBookingRequests(List<BookingRequest> bookingReqList, String timings)
		...
		for (BookingRequest request : bookingReqList) {
			//TODO declare the variable outside
			...
			if (checkIfInOfficeTimings(request, timings)) {
				...
				if (!BookingCalender.fixedBooking.containsKey(BookingDateKey)) {
					employeebookings.add(book);
					BookingCalender.fixedBooking.put(BookingDateKey, employeebookings);
				} else {
					employeebookings = BookingCalender.fixedBooking.get(BookingDateKey);
					for (Booking already : employeebookings) { 
	...
	/**
	 * Checks if booking request falls in office timing or not.
	 * 
	 ...
	 * @throws ParseException
	 */
	//TODO: rename the method to a logically correct name
